...
// read data from SPIdev from specific register address [reg address has just 16 bits]
int spi_read_reg16(spi_t *self, uint16_t reg_addr, void *rx_buf, int len, int swap)
{
  int retv ;
  uint16_t reg_addr_s;  
  ...
  if(swap){
    reg_addr_s = (reg_addr << 8) | ((reg_addr >> 8) & 0xFF);
  }else{
    reg_addr_s = reg_addr;
  }
  	
  // Write message for register address
  xfer[0].tx_buf = (__u64)&reg_addr_s;   // output buffer 
  xfer[0].rx_buf = (__u64)0;             // input buffer
  xfer[0].len = (__u32)sizeof(reg_addr); // length of data to read
...
//----------------------------------------------------------------------------
// write data to SPIdev to specific register address [reg address has just 16 bits]
int spi_write_reg16(spi_t *self, uint16_t reg_addr, const void *tx_buf, int len, int swap)
{
  int retv;
  uint16_t reg_addr_s;  
  struct spi_ioc_transfer xfer[2] = {0};
